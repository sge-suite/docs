...
`end_date_calculation.version` identifica a fórmula e `safety_margin_days` é inteiro não negativo, inicialmente `7`. Ambos são congelados no estágio. A regra completa está definida por este contrato e pelo [serviço de cálculo de término](doc:service-internshipenddatecalculator).
