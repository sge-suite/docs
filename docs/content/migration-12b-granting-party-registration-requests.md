...
A aprovação futura selecionará ou criará `granting_parties` e `addresses` em transação e associará a concedente resultante ao pedido. CNPJ repetido não impede unidades distintas. A solicitação de estágio só poderá ser aceita após substituir a referência pendente pela concedente aprovada.
