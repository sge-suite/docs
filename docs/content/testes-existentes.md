...
| Cursos | `tests/Feature/CourseTest.php` | Schema PostgreSQL das Migrations 09 e 10, FKs, dois coordenadores opcionais e elegíveis, campus, vínculos discentes por curso, desativação, exclusão restrita e rollback/reaplicação das Migrations 11, 10 e 09. |
...
| Pedidos de supervisor | `tests/Feature/SupervisorRegistrationRequestTest.php` | Schema PostgreSQL, FK do vínculo resultante e bloqueio de exclusão física, casts, relação com Affiliation, estados `Draft` e não-`Draft`, validação e obrigatoriedade, snapshot JSONB, motivos de decisão, aprovação vinculada a supervisor e rollback/reaplicação. |
